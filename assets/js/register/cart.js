jQuery(document).ready(function($) { 
	window.POS_CART = {
		...
		set_quantity : function ( cart_item_key, quantity, refresh_totals  ) {
			...
			if (fee_in_cart = CART.fee_in_cart(cart_item_key)) { 
				var fee_item_meta = CART.cart_contents[ cart_item_key ].data.meta;				
				if (typeof fee_item_meta !== 'undefined') { 
					var fee_qty = (fee_item_meta['product-fee-multiplier'] == 'yes') ? quantity : CART.cart_contents[ fee_in_cart ]['quantity'];
					$('#'+fee_in_cart+' .quantity').val(fee_qty);
					CART.cart_contents[ fee_in_cart ]['quantity'] = fee_qty;	
				}
			}
		...
		addToCart : function (product_data, product_id, quantity, variation_id, variation, cart_item_data) {
			...
			try{
				...
				var product_meta = product_data.meta;
				if (CART.product_has_fees(product_meta)) { 					
					var fee_data = CART.product_get_fees(product_meta), 
						fee_in_cart = CART.fee_in_cart(cart_item_key),
						fee_quantity = (fee_data.multiplier == 'yes') ? CART.cart_contents[ cart_item_key ]['quantity'] : 1;
						
					if (fee_data.amount.includes('%')) { 
						fee_data.amount = (price * (fee_data.amount.replace('%', '') / 100)).toFixed(2);
					}
					if (fee_in_cart) { 
						CART.set_quantity( fee_in_cart, fee_quantity, false );
						$('#'+fee_in_cart+' .quantity').val(fee_quantity);
					}
					else { 					
						var fee_product_data = clone(pos_custom_product);
						fee_product_data.title = fee_data.name;
						fee_product_data.price = fee_data.amount;
						fee_product_data.regular_price = fee_product_data.price;
						fee_product_data.taxable = false;
						fee_product_data.tax_status = "none";
						fee_product_data.fee_parent_key = cart_item_key;
						fee_product_data.fee_parent_id = product_id;
						CART.addToCart(fee_product_data, fee_product_data.id, fee_quantity, 0, {});	
					}	
				}
		...
		remove_cart_item : function ( cart_item_key ) {
			if ( typeof CART.cart_contents[ cart_item_key ] != 'undefined' ) {
				...
				var fee_in_cart = CART.fee_in_cart(cart_item_key);
				if (fee_in_cart) { 
					delete CART.cart_contents[ fee_in_cart ];
					$('#'+fee_in_cart).remove();	
				}
		...
		*/
		...
		calculate_totals : function () {
			...
			CART.total = max( 0, r );
			CART.subtotal_ex_tax += CART.fee_total; 
		...
		fee_in_cart : function(product_cart_key) {
			var result = false;		
			$.each(CART.get_cart(), function(cart_item_key, cart_item) {
				if (typeof cart_item.data.fee_parent_key !== 'undefined' && cart_item.data.fee_parent_key == product_cart_key) {
					result = cart_item_key;
				} 
			});	
			return result;	
		},
		product_has_fees : function(meta) { 
			if (typeof meta == 'object') {
				if (typeof meta['product-fee-name'] != 'undefined' && typeof meta['product-fee-amount'] != 'undefined') { 
					return true;
				}
			}
			return false;
		},
		product_get_fees : function(meta) { 
			if (CART.product_has_fees(meta) !== false) {
				var fee_data = { 
						'name': 		meta['product-fee-name'], 
						'multiplier':	meta['product-fee-multiplier'],
						'amount':		meta['product-fee-amount']
					}
				return fee_data;
			}
			else { 
